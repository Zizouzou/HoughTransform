    ...
    #Le changement d'echelle
        for s in range(1, 51):
            for dr in r_table[phi]:
                I_refs = I_phi + (s * dr[0]) // 5
                J_refs = J_phi + (s * dr[1]) // 5
...
def findMaxima(accum, numOfMaxima, delta):
    ...
    for _ in range(numOfMaxima):
        ...
        x, y, s = maxima[0], maxima[1], maxima[2]
        maximas.append((x, y, s))
        accum[x - delta: x + delta, y - delta: y + delta, s-2:s+2] = 0
